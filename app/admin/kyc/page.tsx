...
import { useGetKYCRequestsQuery, useApproveKYCMutation, useRejectKYCMutation } from "@/store/services/adminApi";
...
export default function KYCManagement() {
  ...
  const handleRejectKYC = async (requestId: number) => {
        ...
        <div className="mt-6 space-y-6">
          {kycRequests.length === 0 ? (
            <div className="bg-white p-6 text-center rounded-lg shadow">
              <p className="text-gray-500">Không tìm thấy yêu cầu KYC nào</p>
            </div>
          ) : (
              ...
              <div key={request.id} className="bg-white p-6 rounded-lg shadow-md">
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-col sm:flex-row sm:space-x-6">
                      <span className="text-gray-500 text-sm">
                        Ngày gửi: {formatDateTime(request.submitted_at)}
                      </span>
                      {request.verified_at && (
                        <span className="text-gray-500 text-sm">
                          Ngày xác minh: {formatDateTime(request.verified_at)}
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {request.rejection_reason && (
                  <div className="mt-4 p-3 bg-red-50 rounded text-sm text-red-700">
                    <span className="font-semibold">Lý do từ chối:</span> {request.rejection_reason}
                  </div>
                )}
