...
export default function StatisticsPage() {
  ...
  const [stats, setStats] = useState({
    ...
    rejectedUsers: 0,
    verificationRate: 0,
    monthlyStats: [] as { month: string; verified: number; pending: number; rejected: number }[],
    dailyStats: [] as { date: string; verified: number; pending: number; rejected: number }[]
  });
    ...
    const fetchStatistics = async () => {
      ...
      try {
        ...
        const monthlyStats = [
          ...
          { month: "05/2024", verified: accounts.length, pending: 8, rejected: 3 }
        ];
        ...
        for (let i = 29; i >= 0; i--) {
          ...
          const verified = i === 0 ? accounts.length : Math.floor(Math.random() * 3);
        ...
        setStats({
          ...
          pendingUsers: 8,
          rejectedUsers: 3,
          verificationRate: Math.round((accounts.length / (accounts.length + 8 + 3)) * 100),
          monthlyStats,
          dailyStats
        });
      } catch (error) {
      ...
      {loading ? (
        ...
        <>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      ...
                      <dd>
                        <div className="text-lg font-medium text-gray-900">{stats.verifiedUsers}</div>
                      </dd>
                    </dl>
                  </div>
                </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-5 w-0 flex-1">
                    <dl>
                      ...
                      <dd>
                        <div className="text-lg font-medium text-gray-900">{stats.rejectedUsers}</div>
                      </dd>
                    </dl>
                  </div>
                </div>
          ...
          <div className="mt-8 bg-white shadow rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900">Tỷ lệ xác minh KYC</h2>
            <div className="mt-4">
              <div className="relative pt-1">
                ...
                <div className="overflow-hidden h-2 mb-4 text-xs flex rounded bg-orange-200">
                  <div style={{ width: `${stats.verificationRate}%` }} className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-orange-500"></div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 bg-white shadow rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900">Thống kê theo tháng</h2>
            <div className="mt-4">
                      ...
                      <div className="relative w-full flex flex-col items-center">
                        <div className="h-32 w-full flex flex-col-reverse">
                          <div
                            ...
                            style={{ height: `${(month.rejected / (month.verified + month.pending + month.rejected)) * 100}%` }}
                          ></div>
                          <div
                            className="bg-yellow-500"
                            style={{ height: `${(month.pending / (month.verified + month.pending + month.rejected)) * 100}%` }}
                          ></div>
                          <div
                            className="bg-green-500"
                            style={{ height: `${(month.verified / (month.verified + month.pending + month.rejected)) * 100}%` }}
                          ></div>
                        </div>
              ...
              <div className="mt-4 flex justify-center">
                <div className="flex items-center mr-4">
                  ...
                  <span className="text-sm text-gray-600">Đã xác minh</span>
                </div>
                <div className="flex items-center mr-4">
          ...
          <div className="mt-8 bg-white shadow rounded-lg p-6">
            <h2 className="text-lg font-medium text-gray-900">Thống kê 7 ngày gần nhất</h2>
                        ...
                        {stats.dailyStats.slice(-7).map((day, index) => (
                          <tr key={index}>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">{day.verified}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">{day.pending}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                              <div className="text-sm text-gray-900">{day.rejected}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {day.verified + day.pending + day.rejected}
                            </td>
                          </tr>
                        ))}
                      </tbody>
