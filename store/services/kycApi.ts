...
export const kycApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getFaceVerificationStatus: builder.query<FaceVerificationStatus, void>({
      query: () => '/kyc/face-verification-status',
      providesTags: ['FaceStatus'],
    }),
    
    // Tải lên ảnh CCCD
    uploadIdCard: builder.mutation<{ success: boolean; message: string }, { file: File; isFront: boolean }>({
      query: ({ file, isFront }) => {
        const formData = new FormData();
        formData.append('id_card', file);
        formData.append('is_front', isFront.toString());
        
        return {
          url: '/kyc/upload-id-card',
          method: 'POST',
          body: formData,
        };
      },
    ...
    // Xác minh khuôn mặt
    verifyFaceMatch: builder.mutation<{ success: boolean; message: string }, File>({
      query: (file) => {
        const formData = new FormData();
        formData.append('selfie', file);
        
        return {
          url: '/kyc/verify-face-match',
          method: 'POST',
          body: formData,
        };
      },
    ...
    getVerifiedAccounts: builder.query<{ accounts: VerifiedAccount[] }, void>({
